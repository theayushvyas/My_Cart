...
def create_db():
            ...
            c.execute("""CREATE TABLE IF NOT EXISTS USERCART(cart_id integer primary key ,product_name varchar COLLATE NOCASE,amount integer,username integer,FOREIGN KEY(username)REFERENCES USERS(username))""")
            c.execute("""CREATE TABLE IF NOT EXISTS USER_STATUS(id integer primary key , username integer)""" )
